...
- **Hue tendency / saturation** — grounded in **Radix Colors**' 12-step model: step 9 has the *highest chroma* (purest, for solid vivid backgrounds), step 10 is the darker hover variant. So `playful-vivid` / `bold-tech` reaching for saturated hues = high-chroma (Radix step ~9), and `premium-editorial` "muted, near-mono" = lower-chroma steps, are grounded. *Which* hue family a mood picks (warm amber for `friendly-warm`, tech green/cyan for `technical-dev`) is still **team convention** (colour association, culture-dependent).
- **Shadow strength / border-width** — **team convention**. The *principle* of a consistent elevation scale is standard (Material / IBM Carbon), but the specific per-mood strength here is our opinionated default; override freely per brand. (No specific Carbon shadow dp values are cited because they were not verified.)

> Refs: [Material 3 — corner radius scale](https://m3.material.io/styles/shape/corner-radius-scale) · [Tailwind — border-radius](https://tailwindcss.com/docs/border-radius) · [Radix Colors — understanding the scale](https://www.radix-ui.com/colors/docs/palette-composition/understanding-the-scale) · Refactoring UI (Wathan & Schoger) — typography/letter-spacing chapter. Values verified 2026-07; mood→value pairings are this skill's editorial defaults.
...
primitive:
  ...
  breakpoints:  { 320, 340, 360, 768, 1024, 1280, 1440, 1920 }          # px — 320/340 back the mobile-first 2xs/xs semantic tiers, always generated by default
  shadow:
...
**Responsive variance — default for `heading.*`, opt-in elsewhere (v6.4+):** mobile-first is the assumed baseline for every project (no need to ask). Every `heading` role (`h1`–`h4`) MUST include a `responsive:` sibling key by default — large display type reliably needs to shrink on small viewports, confirmed against real shipped apps that ship separate mobile/desktop heading scales for exactly this reason. `body`/`label`/`caption` stay non-responsive by default (add `responsive:` to them too if a specific project needs it — same mechanism, just not generated automatically).

`responsive:` holds **diff-only overrides per breakpoint tier** — same merge philosophy as component variant/size/state (last declared tier at or below the viewport wins, mobile-first cascade):
  ...
  typography:
    heading:
      h1:
        ...
        tracking:    '{primitive.typography.tracking.tight}'
        responsive:                                          # REQUIRED for heading.* by default — mobile-first baseline
          2xs:                                                 # diff-only — only keys that change at this tier
            size:        '{primitive.typography.size.3xl}'
            line-height: '{primitive.typography.line-height.3xl}'
          sm:
...
Rules:
- `responsive:` is **required by default on every `heading.*` role** — do not skip it or ask the user first. Optional (opt-in, same mechanism) on `body`/`label`/`caption`.
...
### Breakpoint roles (required — mobile-first base, v6.4+)

T-shirt, 7 tiers, **all always generated by default**: `2xs, xs, sm, md, lg, xl, 2xl` — each `{primitive.breakpoints.N}`-ref. No need to ask the user whether the project is mobile-first — assume it is (the default target today) and always include the fine-grain mobile tiers `2xs`/`xs` below `sm`. Do not skip them or gate them behind a question.
